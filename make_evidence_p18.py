...
def calculate_edit_distance(word1, word2, threshold=0.9):
    # return jellyfish.levenshtein_distance(word1, word2) <= threshold
    # return SequenceMatcher(None, word1, word2).ratio() >= threshold
    # return jellyfish.jaro_similarity(word1, word2) >= threshold
    return jellyfish.jaro_winkler_similarity(word1, word2) >= threshold

def mask_similar_words(question, reference_list, mask_string):
    words = question.split()
    masked_words = words.copy()
    
    for n in range(6, 0, -1):  # Start from 6-gram down to 1-gram
        for i in range(len(masked_words) - n + 1):
            n_gram = " ".join(masked_words[i:i + n])
            for ref_word in reference_list:
                if calculate_edit_distance(str(n_gram).lower(), str(ref_word).lower()):
                    masked_words[i:i + n] = [mask_string]
                    break
...
def question_masking(json_data, table_json):
    for data in json_data:
        ...
        masked_question = mask_similar_words(question, schema_list, "<schema>")
        masked_question = mask_similar_words(masked_question, value_list, "<value>")
