...
There are some precautions in generating evidence. When generating evidence, please refer to the precautions below.
# Precaution 1. If an evidence includes a date or time, pay attention to the format. Since the format indicating the date or time may be different for each column, it should be generated in the same format by referring to the value sample in the column.
# Precaution 2. If an evidence includes between keyword, pay attention to the range of values. When expressed as between A and B, A is included in the range, but B is not. With this in mind, an evidence should be created.
# Precaution 3. For DB values, you should pay attention to case-sensitive. DB values are case-sensitive. Therefore, if evidence contains DB values, you should pay attention to case-sensitive.
"""
                    ...
                    if row:
                        ...
                        try:
                            ...
                            if "blob" in column_values:
                                ...
                                # sql = f"SELECT distinct case when `{row[0].strip()}` is null then 'null' else `{row[0].strip()}` end FROM `{table_name}` limit {num_of_sampling};"
